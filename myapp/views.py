...
def add_packages(request):
    if request.method == "POST":
        ...
        Package.objects.create(
            ...
            other_services=[request.POST.get('food'), request.POST.get(
                'rooms'), request.POST.get('accomodation'), request.POST.get('guide')],
...
def update_package(request, pk):
    ...
    if request.method == "POST":
        ...
        package.other_services = [request.POST.get('food'), request.POST.get(
            'rooms'), request.POST.get('accomodation'), request.POST.get('guide')]
...
def my_packages(request):
    ...
    try:
        ...
        payment = client.order.create(
            {"amount": net_price*100, "currency": "INR", "receipt": "order_rcptid_11"})
        r_pay = net_price*100
...
def deploy(request):
    if request.method == 'POST':
        try:
            path = os.path.join(os.getcwd(), 'myapp', 'deploy.sh')
            print('path', path)
